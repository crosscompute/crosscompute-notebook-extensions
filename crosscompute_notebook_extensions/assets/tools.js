  ...
  function process_notebook(verb, noun) {
    ...
    notebook.events.one('notebook_saved.Notebook', function() {
      $.ajax({
        ...
        if (verb == 'preview') {
          body += '<p>Note that your tool preview will stop working after the notebook session ends. Deploy the tool using the Red Plane to make it available on <a href="https://crosscompute.com" target="_blank">CrossCompute</a>.</p>';
        } else if (verb == 'deploy') {
